"""Bosch Smart Home cloud maintenance / outage discovery (synchronous CLI port).
...
announcement so the CLI can show a specific reason ("Bosch-Wartung 07:00–10:00
...
Behavior-parity note: this is the synchronous CLI port of the HA integration's
``maintenance.py``. Parser logic, state names, DST handling, and the failover
chain are intentionally identical. Use ``requests`` (already a CLI dep) instead
of ``aiohttp``.
"""
...
def _fetch_one(url: str, *, timeout_s: float) -> bytes | None:
    """Fetch *url* synchronously; return body bytes or None on any error."""
    try:
        resp = requests.get(
            url,
...
def fetch_maintenance(timeout_s: float = 8.0) -> MaintenanceWindow | None:
    """Fetch and parse the best-match maintenance/incident announcement.

    Returns None only when ALL primary and fallback sources fail — the caller
    keeps the last cached value rather than flipping state on a transient miss
    of the community site.

    Synchronous version for the CLI (no event loop required). Behavior-parity
    with the HA integration's ``async_fetch_maintenance``.
    """
    best: MaintenanceWindow | None = None
    for url in RSS_FEEDS:
        body = _fetch_one(url, timeout_s=timeout_s)
        if body is None:
            continue
